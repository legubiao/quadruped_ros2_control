...
#include "basic_quadruped_controller/FSM/StateBalanceTest.h"
...
void StateBalanceTest::calcTorque()
{
    ...
    Vec12 current_joints = robot_model_->current_joint_pos_;

    for (int i = 0; i < 4; i++)
    {
        // Calculate torque for each leg using analytical method (more stable)
        Vec3 foot_force = force_feet_body.col(i);
        Vec3 torque = robot_model_->getTorque(foot_force, i);
        ...
        for (int j = 0; j < 3; j++)
        {
            ...
            std::ignore = ctrl_interfaces_.joint_position_command_interface_[i * 3 + j].get().set_value(
                current_joints(i * 3 + j));
        }
    }
}

void StateBalanceTest::setStableGain() const
{
    for (int i = 0; i < 12; i++)
    {
